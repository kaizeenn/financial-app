...
async function processRecurringTransactions() {
  try {
    ...
    for (const recurring of dueRecurring) {
      try {
        // Prepare description
        const description = recurring.description || `Transaksi berulang otomatis`;

        // Insert transaction based on type
        if (recurring.type === 'income') {
          await db.query(
            `INSERT INTO income (user_id, category_id, account_id, amount, description, entry_date)
             VALUES (?, ?, ?, ?, ?, CURDATE())`,
            [
              recurring.user_id,
          ...
          await db.query(
            `INSERT INTO expense (user_id, category_id, account_id, amount, description, entry_date)
             VALUES (?, ?, ?, ?, ?, CURDATE())`,
            [
              recurring.user_id,
              recurring.category_id,
